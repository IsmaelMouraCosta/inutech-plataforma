...
import { getArticleBySlug, getAllArticles, formatDate, getCategoryLabel } from '@/lib/revista'
import { Calendar, Clock, Users, BookOpen, ArrowLeft, Share2, ExternalLink } from 'lucide-react'
...
export async function generateMetadata({ params }: ArticlePageProps): Promise<Metadata> {
  ...
  if (!article) {
    return {
      title: 'Artigo não encontrado - iNuTech Revista',
    }
  }

  return {
    title: `${article.title} - iNuTech Revista Científica`,
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <article className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-lg">
              ...
              <header className="mb-8">
                ...
                {article.subtitle && (
                  <h2 className="text-xl text-gray-600 dark:text-slate-300 italic mb-4">
                    {article.subtitle}
                  </h2>
                )}
              ...
              {/* Abstract */}
              <div className="mb-8 p-6 bg-blue-50 dark:bg-blue-900/10 rounded-lg border-l-4 border-blue-500">
                ...
                <div className="mb-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {article.keywords.map((keyword) => (
                      <span 
                        key={keyword}
                        className="inline-flex px-3 py-1 text-sm font-medium bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-slate-200 rounded-full"
                      >
                        {keyword}
                      </span>
                    ))}
              ...
              <footer className="mt-12 pt-8 border-t border-gray-200 dark:border-slate-700">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-500 dark:text-slate-400">
                    <p>Publicado em {formatDate(article.date)}</p>
                    <p>Volume {article.volume}, Edição {article.issue}</p>
                    {article.pages && <p>Páginas: {article.pages}</p>}
                  </div>
        ...
        <section className="py-16 px-4 sm:px-6 lg:px-8 bg-gray-50 dark:bg-slate-900">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {/* Aqui você pode adicionar artigos relacionados baseados em categoria e keywords */}
              <div className="bg-white dark:bg-slate-800 rounded-xl p-6 text-center">
                <p className="text-gray-600 dark:text-slate-400">
                  Em breve: artigos relacionados baseados em categoria e palavras-chave.
                </p>
              </div>
            </div>
          </div>
